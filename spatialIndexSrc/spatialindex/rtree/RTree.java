...
package spatialIndexSrc.spatialindex.rtree;
...
public class RTree implements ISpatialIndex
{
	...
	public int intersectionQuery(final IShape query, final IVisitor v)
	{
...
//		int nearest=-1;
		return rangeQuery(SpatialIndex.IntersectionQuery, query, v);
	}
	...
	public int nearestNeighborQuery(int k, final IShape query, final IVisitor v, final INearestNeighborComparator nnc)
	{
		...
		try
		{
			...
			while (queue.size() != 0)
			{
				...
				else
				{
					...
					nearestNode= first.m_pEntry.getIdentifier();
//					System.out.println(nearestNode+"\t结果");
	...
	public int nearestNeighborQuery(int k, final IShape query, final IVisitor v)
	{
		if (query.getDimension() != m_dimension) throw new IllegalArgumentException("nearestNeighborQuery: Shape has the wrong number of dimensions.");
	...
	private int rangeQuery(int type, final IShape query, final IVisitor v)
	{
		m_rwLock.read_lock();
		int nearestEdge=-1;
		int minDistance=-1;
		Vector<CandidateEdge> candidates=new Vector<CandidateEdge>();
		int candCount=0;
		try
		{
			Stack st = new Stack();
				...
				if (n.m_level == 0)
				{
					...
					for (int cChild = 0; cChild < n.m_children; cChild++)
					{
						...
						if (b)
						{
							...
							candidates.add(new CandidateEdge(candCount, data.getIdentifier(), new VertexPoint(0, data.m_shape.m_pLow[0], data.m_shape.m_pLow[1]), new VertexPoint(1, data.m_shape.m_pHigh[0], data.m_shape.m_pHigh[1])));
							candCount++;
//							System.out.println("结果："+data.m_id+"\t"+data.getIdentifier()+"\t"+data.getShape().getRegionString());
							m_stats.m_queryResults++;
						}
					}
				}
				else
				{
					...
					for (int cChild = 0; cChild < n.m_children; cChild++)
					{
						...
						Node stNode=readNode(n.m_pIdentifier[cChild]);	
		...
		if(candidates.size()>0)
		{
			...
			VertexPoint qVertex=new VertexPoint(0, query.getMBR().m_pLow[0], query.getMBR().m_pLow[1]);
			nearestEdge=candidates.get(0).edgeID;
			
			
//			minDistance=DistanceHelp.getRealDistanceToEdge(qVertex, candidates.get(0));
//			
//			for(CandidateEdge candEdge:candidates)
//			{
//				//不是默认的CandidateEdge
//				if(candEdge.edgeID!=nearestEdge)
//				{
//					int targetDistance=DistanceHelp.getRealDistanceToEdge(qVertex, candEdge);
//					if(targetDistance<minDistance)
//					{
//						minDistance=targetDistance;
//						nearestEdge=candEdge.edgeID;
//					}
//				}
//				
//			}
